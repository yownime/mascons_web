...
const TEST_INFO: Record<string, { title: string; category: string; icon: React.FC<any>; color: string }> = {
  ...
  cfit: { title: 'CFIT', category: 'Tes IQ', icon: Brain, color: 'bg-indigo-500' },
  ...
  kraepelin: { title: 'Kraepelin', category: 'Tes Kerja', icon: Target, color: 'bg-slate-700' },
...
export default async function ParticipantsDashboard() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">Daftar Partisipan</h2>
        <p className="text-slate-500">Pilih kategori tes untuk melihat detail partisipan.</p>
      </div>
        ...
        {Object.entries(TEST_INFO).map(([key, info]) => {
          ...
          return (
            <Link 
              ...
              className="group block bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm hover:shadow-md transition-all hover:border-indigo-500 dark:hover:border-indigo-500"
            >
