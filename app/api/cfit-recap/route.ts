...
export async function GET(req: NextRequest) {
  ...
  const categories =
    scale === 2
      ? ['CFIT_SKALA2', 'cfit_skala2', 'CFIT_Skala2', 'cfit', 'CFIT']
      : ['CFIT_SKALA3', 'cfit_skala3', 'CFIT_Skala3', 'cfit', 'CFIT'];
  ...
  const processed = results
    .filter((r) => {
      // If legacy full CFIT, ensure it has data for this scale
      if (r.category === 'CFIT' || r.category === 'cfit') {
        return testKeys.some((k) => (r.reportData as any)?.[k] !== undefined);
      }
      return true;
    })
    .map((r) => {
